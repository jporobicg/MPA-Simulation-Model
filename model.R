...
normal.t.matrix <- trans.matrix(size.vec, k.g = 0.06985, linf = 213.5, beta.g = 1)
...
for(year in 1: length(years.sim){
    if( year==start.year){
        ...
        abundance[1] <- rec.pat * (1 - diag(normal.t.matrix)* exp(-M))
## think about the zones,  would be a good idea to generalizthis
